namespace UdpTransportBinding
{
    ...
    class SynchronizedQueue<T> : IDisposable where T : class
    {
        ...
        //Count the number of item pending
        ...
        //Lock used to synchronize the queue
        object ThisLock
        {
            get { return itemQueue; }
        }

        //Method used to dequeue an element from the queue.
        ...
        {
            ((SynchronizedQueue<T>)state).Dispatch();
        }
        ...
        {
            Exception exception;
            SynchronizedQueue<T> inputQueue;
            T item;
            ManualResetEvent waitEvent;
            object thisLock = new object();

            public WaitQueueReader(SynchronizedQueue<T> inputQueue)
            {
        ...
        class AsyncQueueReader : AsyncResult, IQueueReader
        {
            ...
            public AsyncQueueReader(SynchronizedQueue<T> inputQueue, TimeSpan timeout, AsyncCallback callback, object state)
                : base(callback, state)
            {
                this.inputQueue = inputQueue;
                if (timeout != TimeSpan.MaxValue)
        ...
        //The real queue containing elements
        class ItemQueue
        {
            Item[] items;
            int head;
            ...
            /*
             * Insert a new item inside the queue. Push an element in tail position.
             * If the queue is full the method double the dimension of the queue reinserting all the elements in the
             * same order. The method increase the totalCount.
            */
            ...
            /*
             * Dequeue the head element of the queue. It pops an element. Insert an empty element at head position
             * and then moves the head pointer one slot forward.
            */
            ...
            /*
             * Enqueue an element already in pending state. It increases the pendingCount
            */
            ...
            //check if the queue have a pending element.
            public bool HasAvailableItem
            {
                get { return totalCount > pendingCount; }
            }

            //Check if the queue have elements
